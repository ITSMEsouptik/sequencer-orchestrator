...
@Component
public class AccessioningHandler implements SagaStepHandler {

    @Override
    public EventType handlesEvent() {
        return EventType.PRODUCT_ACCESSIONED;
    }

    @Override
    public SagaResult handle(SequencerEvent event, TherapyOrder order) {
        return SagaResult.success(OrderStatus.MANUFACTURING, EventType.MANUFACTURING_STARTED);
    }
}