...
@Component
public class ReleaseHandler implements SagaStepHandler {

    @Override
    public EventType handlesEvent() {
        return EventType.PRODUCT_RELEASED;
    }

    @Override
    public SagaResult handle(SequencerEvent event, TherapyOrder order) {
        return SagaResult.success(OrderStatus.IN_TRANSIT_OUTBOUND, EventType.OUTBOUND_SHIPMENT_DISPATCHED);
    }
}
